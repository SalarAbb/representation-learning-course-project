...
class supervised_betavae_model(object):
    ...
    def compute_loss(self):
        ...
        return self.recons_loss + self.beta * self.latent_loss + self.mu * (self.loss_supervised)
        ...
        if self.train_restore == 'train':        
            ...
            for epoch in range(1,self.num_epochs+1):
                ...
                for iteration in range(1,self.num_iterations_per_epoch+1):
                    ...
                    self.sess.run(self.operation_train, feed_dict = { self.x: batch_x, self.y:batch_y, self.is_training: True, self.beta: beta_this} ) 
                    # plot
                    if self.sw_plot == 1:
                    ...
                    if iteration % self.print_info_step == 0 or iteration == self.num_iterations_per_epoch:
                        loss_this, recons_loss_this, latent_loss_this, loss_supervised= self.sess.run([self.loss_train,self.recons_loss,self.latent_loss,self.loss_supervised], feed_dict = {self.x: batch_x, self.y:batch_y, self.is_training: True, self.beta: beta_this})
                        # print info                        
                ...
                # run gradient descent
                loss_this, recons_loss_this, latent_loss_this = self.sess.run([self.loss_train,self.recons_loss,self.latent_loss], feed_dict = {self.x: batch_x_test, self.y:batch_y_test, self.is_training: False, self.beta: beta_this})
    ...
    def plot_images_from_latent(self,batch_image,batch_latent_manipulated):
        ...
        for img_x in range(5):
            for img_y in range(3):
                img_index = img_y*5 + img_x
                ax = plt.subplot2grid( (5,7) , (img_x,img_y) )
                ax.imshow(255*np.squeeze(x_test[img_index,:,:]) )
                ax.set_title('True {}'.format(img_index))
                ax.set_xticks([],[])    
                ax.set_yticks([],[])
                ax = plt.subplot2grid( (5,7) , (img_x,img_y+4) )
                ax.imshow(255*np.squeeze(x_test_reconsutructed[img_index,:,:]) )
                ax.set_title('Recons. {}'.format(img_index))
                ax.set_xticks([],[])    
                ax.set_yticks([],[])  
    ...
    ###########################################################################################################################    
    ################################################### SUPERVISED GAMMA VAE ##################################################
    ###########################################################################################################################

class supervised_gammavae_model(object):
    # this class creates the RNN model for movement generation
    ...
    def __init__(self,data_set,gamma,capacity_schedule,mu,options):
        ...
        self.save_main_folder = '{}/models_saved/gamma_vae/trained_model_gamma{:.1f}_cschedule_{}to{}{}'.format(self.save_global_directory,self.gamma, int( np.min(self.capacity_schedule) ), int( np.max(self.capacity_schedule) ), string_dim )
        # self.save_directory = options['save_directory'] # directory to save the results
